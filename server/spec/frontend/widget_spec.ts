...
test('widget creation', (t) => {
  const widget = buildWidget({ command: '', id: 'foo', css: 'background: red' });
...
test('defaults', (t) => {
  let implementation = buildWidget({ command: '', id: 'foo', css: '' })
    .implementation();
...
test('internal api', (t) => {
  const api = buildWidget({ command: '', id: 'foo', css: '' })
    .internalApi();
...
test('running commands', (t) => {
  ...
  const instance = buildWidget({ id: 'foo', command: 'command', css: ''});
  const widget = instance.implementation();
...
test('manually running commands', (t) => {
  const widget = buildWidget({ id: 'foo', command: '', css: ''}).internalApi();
  const server = makeFakeServer();
  server.respondToRun('some output');
...
test('standard rendering', (t) => {
  ...
  const domEl = instance.create();
  const contentEl = domEl.querySelector('.widget');
  t.equal(
  ...
  const internalApi = instance.internalApi();
  internalApi.stop();
  clock.tick(100);
  clock.tick(100);
  t.equal(
