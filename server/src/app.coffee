...
perfServer = require('./perfServer')
widgetControlServer = require('./widgetControlServer')
...
module.exports = (port, widgetPath, settingsPath, publicPath, options, callback) ->
  ...
  middleware = connect()
    ...
    .use(widgetControlServer((action) ->
      store.dispatch(action)
      dispatchToRemote(action)
    ))
  ...
  server.listen port, host, (err) ->
    try
      ...
      messageBus = MessageBus(
        server: server,
        verifyClient: (info) ->
          info.req.headers.host == allowedHost && (info.origin == allowedOrigin || info.origin == 'Uebersicht')
      )
